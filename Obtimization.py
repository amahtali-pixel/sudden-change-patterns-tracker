...
class PatternMatcher:
    def __init__(self, device='cuda', tolerance=0.38, batch_size=32):
    ...
    def process_single_image(self, image, true_label, image_idx):
        ...
        # Collect patterns for analysis from ALL training images
    ...
    def run(self, num_images=50000):
        """Main execution method - now using training data"""
        print(f"{BLUE}Loading TRAINING images...{ENDC}")
        train_dataset = datasets.MNIST(root='./data', train=True, download=True)
        train_images = train_dataset.data[:num_images].float() / 255.0
        train_labels = train_dataset.targets[:num_images]
        ...
        for i in range(len(train_images)):
            result = self.process_single_image(train_images[i], train_labels[i].item(), i)
            all_results.append(result)

            # Show progress every 100 images
            if (i + 1) % 100 == 0:
                elapsed = time.time() - start_time
                print(f"{BLUE}Processed {i + 1}/{len(train_images)} training images "
                      f"({elapsed:.2f} seconds elapsed){ENDC}")

            # Save pattern analysis periodically to avoid memory issues
            if (i + 1) % 5000 == 0:
                self.save_pattern_analysis()
                print(f"{GREEN}Saved patterns after {i + 1} images{ENDC}")
    ...
    def print_final_results(self, all_results, start_time):
        ...
        # Save results
        with open('pattern_match_results_training.pkl', 'wb') as f:
            pickle.dump(all_results, f)
        print(f"{GREEN}Results saved to pattern_match_results_training.pkl{ENDC}")


def main():
    """Main function"""
    # Initialize pattern matcher
    matcher = PatternMatcher(device='cuda', tolerance=0.38, batch_size=32)

    # Run the pattern matching on training data
    results = matcher.run(num_images=50000)
