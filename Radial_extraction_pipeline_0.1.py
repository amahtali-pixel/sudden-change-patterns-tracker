...
# Digit pattern extraction and clustering pipeline for MNIST recognition
#
# This program processes MNIST digits by:
# 1. Recentering each digit within its 20x20 bounding box
# 2. Extracting "double rotary" edge patterns (8-value features around edges)
# 3. Clustering similar patterns in each of 16 spatial zones (4x4 grid)
# 4. Saving clustered patterns for each digit class (0-9)
#
# The clustered patterns serve as reference templates for downstream
# digit recognition via pattern matching algorithms.
#
# Key features:
# - Works on one digit class at a time
# - Uses GPU acceleration for edge detection and pattern extraction
# - Clusters with tolerance=0.1 (strict similarity threshold)
# - Outputs pickled pattern files per class for use in recognition systems
...
def cluster_patterns_simple(patterns, tolerance=0.1):  # CHANGED: from 0.12 to 0.1
...
def save_clustered_patterns(clustered_patterns, class_digit, filename_prefix="clustered_01"):  # CHANGED: prefix to include tolerance
    ...
    sample_filename = f"clustered_01_{class_digit}_samples.txt"  # CHANGED: to match tolerance value
    with open(sample_filename, 'w') as f:
        ...
        f.write(f"Clustering tolerance: 0.1\n\n")  # ADDED: explicit tolerance in sample file
...
def process_class(class_digit, device='cuda', tolerance=0.1):  # CHANGED: default tolerance explicitly set to 0.1
...
def main():
    ...
    tolerance = 0.1  # CHANGED: from 0.12 to 0.1
